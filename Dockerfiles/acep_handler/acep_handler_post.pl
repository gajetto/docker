...
our $queries={
	...
	save_status=>'INSERT INTO sendReportsStatus (status,product,report_id) values (?,?,?) '
};
...
sub parse_raw_data: Override{
	...
	#time = time;
	$timelogger{2}=time;
	my $answer=HTTPS::send($parameters);
	if ($answer){
		save_answer($answer,$sth,$id);
		$timelogger{3}=time;
		...
		$timelogger{4}=time;
#	        my $oneT=$timelogger{1}-$timelogger{0};
#                my $twoT=$timelogger{2}-$timelogger{1};
#	               my $dT=$timelogger{3}-$timelogger{2};
#		my $tT=$timelogger{4}-$timelogger{3};
#		my $allT=$timelogger{4}-$timelogger{0};
 #               open F,">>/home/data$$.csv";
  #              print F $oneT.",".$twoT.",".$dT.",".$tT.",".$allT."\n";
#                close F;

}

sub save_answer{
	my ($json_text,$sth,$id)=@_;
	my  $json = JSON->new->allow_nonref;
	my $data = $json->decode( $json_text );
	my $status=($data->{'status'} eq 'ok')?1:0;
	$sth->{'save_status'}->execute($status,$data->{'product'},$id);
}
...
sub build_parameters: Override{
	...
	my $parse = new XML::Parser(Style => 'Debug');
   	#open FILE, ">/reports/report_$id";
	#print FILE $problem_report_xml;
	#close FILE;
...
sub get_config_db{
        ...
        $config=Config::INI::Reader->read_file(CONFIGPATH);
